...
const LinksModal = ({ state, onClose, nodes, edges, onUpdateLink, onDeleteLink }) => {
  ...
  const getRelationDetails = (edge) => {
    ...
    // Título dinámico bonito (Ej: "Padre", "Ex-esposo")
  ...
  const getSuggestions = (edgeToEdit) => {
     ...
     if(isSourceFrom) return []; // Si edito un hijo, no sugiero parejas por ahora
     
     // Si edito un padre, sugiero las parejas del OTRO padre
             ...
             nodeEdges.map(edge => {
               ...
               return (
                 <div key={edge.id} className={`bg-white rounded-2xl border transition-all duration-200 overflow-hidden shadow-sm ${isExpanded ? 'border-purple-300 ring-4 ring-purple-50' : 'border-gray-100 hover:border-purple-200'}`}>
                    ...
                    {/* Header del Link (Clicable para expandir) */}
                    <div 
...
export default function FamilyTreeApp() {
  ...
  // --- LÓGICA DE ÁRBOL, LÍNEAS Y PAREJAS ---

  const handleLineClick = (edgeId, sourceNodeId) => {
     setLinksModalState({ isOpen: true, nodeId: sourceNodeId, expandedEdgeId: edgeId });
  };

  const getPartnerOffset = (sourceId) => {
      const existing = edges.filter(e => (e.from === sourceId || e.to === sourceId) && ['spouse', 'ex_spouse', 'partner'].includes(e.type)).length;
  ...
  const confirmAddChild = (sourceId, partnerId) => {
      ...
      newEdges.push({ id: generateId(), from: sourceId, to: childId, type: 'parent', label: 'Biológico' });
      if (partnerId) {
          newEdges.push({ id: generateId(), from: partnerId, to: childId, type: 'parent', label: 'Biológico' });
      }
  ...
  const handleNodeAction = (action) => {
    ...
    if (action === 'add_parents') {
      ...
      newEdges.push(
        { id: generateId(), from: fatherId, to: sourceNode.id, type: 'parent', label: 'Biológico' },
        { id: generateId(), from: motherId, to: sourceNode.id, type: 'parent', label: 'Biológico' },
        { id: generateId(), from: fatherId, to: motherId, type: 'partner', label: 'Casado/a' }
      );
    } 
    else if (action === 'add_child') {
      const partners = edges
         .filter(e => (e.from === selectedNodeId || e.to === selectedNodeId) && ['spouse', 'ex_spouse', 'partner'].includes(e.type))
      ...
      return;
       ...
       newEdges.push({ id: generateId(), from: sourceNode.id, to: spouseId, type: 'partner', label: 'Casado/a' });
    }
       ...
       newEdges.push({ id: generateId(), from: sourceNode.id, to: exId, type: 'partner', label: 'Divorciado' });
    }
  ...
  const handleUpdateLink = (edgeId, updates) => {
     const edgeToUpdate = edges.find(e => e.id === edgeId);
     if (!edgeToUpdate) return;

     let newNodes = [...nodes];
     let finalTargetId = edgeToUpdate.from === linksModalState.nodeId ? edgeToUpdate.to : edgeToUpdate.from;

     if (updates.targetId) {
        if (updates.targetId === 'NEW') {
           finalTargetId = generateId();
           const sourceNode = nodes.find(n => n.id === linksModalState.nodeId);
           
           let dy = 0; let dx = 100;
           if (edgeToUpdate.type === 'parent') {
              dy = edgeToUpdate.from === linksModalState.nodeId ? 150 : -150;
     ...
     const newEdges = edges.map(e => {
        if (e.id === edgeId) {
           ...
           if (updates.targetId !== undefined) {
              if (e.from === linksModalState.nodeId) updatedEdge.to = finalTargetId;
  ...
  return (
    ...
    >
      ...
      {/* MODAL GESTIÓN DE VÍNCULOS AVANZADO */}
      <LinksModal
      ...
      <svg className="w-full h-full pointer-events-none">
        <g transform={`translate(${transform.x}, ${transform.y}) scale(${transform.k})`}>
          ...
          {edges.map(edge => {
            ...
            return (
               <g key={edge.id}>
                 {/* Hitbox expandido para ser clickeado fácilmente con el dedo (Grosor: 30px) */}
                 <path 
                    d={d} 
                    stroke="transparent" 
                    strokeWidth="30" 
                    className="pointer-events-auto cursor-pointer hover:stroke-purple-300/30 transition-colors" 
                    onClick={(e) => { e.stopPropagation(); handleLineClick(edge.id, from.id); }} 
                 />
